                ...
                case EVENT_BM_START_MUSIC:
                    ...
                    switch (me->GetAreaId())
                    {
                        ...
                        case THUNDERBLUFF:
                            if (rnd == 0)
                            {
                                me->PlayDistanceSound(EVENT_BREWFESTGOBLIN01);
                                musicTime = EVENT_BREWFESTGOBLIN01_TIME;
                            }
                            else if (rnd == 1)
                            {
                                me->PlayDistanceSound(EVENT_BREWFESTGOBLIN02);
                                musicTime = EVENT_BREWFESTGOBLIN02_TIME;
                            }
                            else
                            {
                                me->PlayDistanceSound(EVENT_BREWFESTGOBLIN03);
                        ...
                        case DARNASSUS:
                            if (rnd == 0)
                            {
                                me->PlayDistanceSound(EVENT_BREWFESTDWARF01);
                                musicTime = EVENT_BREWFESTDWARF01_TIME;
                            }
                            else if (rnd == 1)
                            {
                                me->PlayDistanceSound(EVENT_BREWFESTDWARF02);
                                musicTime = EVENT_BREWFESTDWARF02_TIME;
                            }
                            else
                            {
                                me->PlayDistanceSound(EVENT_BREWFESTDWARF03);
                    ...
                    _events.ScheduleEvent(EVENT_BM_START_MUSIC, 5s); // Every 5 second's SMSG_PLAY_MUSIC packet (PlayDirectSound) is pushed to the client
                    break;
                default:
                    break;
                }
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_MM_START_MUSIC:
                    {
                        ...
                        {
                            if (player->GetTeamId() == TEAM_HORDE)
                                me->PlayDistanceSound(EVENTMIDSUMMERFIREFESTIVAL_H, player);
                            else
                                me->PlayDistanceSound(EVENTMIDSUMMERFIREFESTIVAL_A, player);
                        }
                        _events.ScheduleEvent(EVENT_MM_START_MUSIC, 5s); // Every 5 second's SMSG_PLAY_MUSIC packet (PlayDirectSound) is pushed to the client (sniffed value)
                        break;
                    }
                default:
                    break;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                ...
                case EVENT_DFM_START_MUSIC:
                    ...
                    _events.ScheduleEvent(EVENT_DFM_START_MUSIC, 5s);  // Every 5 second's SMSG_PLAY_MUSIC packet (PlayDirectSound) is pushed to the client (sniffed value)
                    break;
                default:
                    break;
                }
