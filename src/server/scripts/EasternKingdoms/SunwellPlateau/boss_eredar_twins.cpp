    ...
    struct boss_sacrolashAI : public BossAI
    {
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            switch (events.ExecuteEvent())
            {
                ...
                {
                    Unit* target = SelectTarget(SELECT_TARGET_MAXTHREAT, 1, 100.0f); // SELECT_TARGET_TOPAGGRO
                    if (!target)
                        target = me->GetVictim();
...
class spell_eredar_twins_blaze : public SpellScriptLoader
{
    public:
        ...
        class spell_eredar_twins_blaze_SpellScript : public SpellScript
        {
            ...
            void Register()
            {
                // @todo OnEffectHitTarget += SpellEffectFn(spell_eredar_twins_blaze_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
            }
        };
