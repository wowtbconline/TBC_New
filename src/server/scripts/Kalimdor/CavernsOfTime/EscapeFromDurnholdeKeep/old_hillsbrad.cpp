...
public:
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        switch(uiAction)
        {
            case GOSSIP_ACTION_INFO_DEF+1:
                ...
                if(EscortAI* pEscortAI = CAST_AI(npc_thrall_old_hillsbrad::npc_thrall_old_hillsbradAI, pCreature->AI()))
                    pEscortAI->Start(true, true, pPlayer->GetGUID());

                CAST_AI(EscortAI, (pCreature->AI()))->SetMaxPlayerDistance(100.0f);//not really needed, because it will not despawn if pPlayer is too far
                CAST_AI(EscortAI, (pCreature->AI()))->SetDespawnAtEnd(false);
                CAST_AI(EscortAI, (pCreature->AI()))->SetDespawnAtFar(false);
                break;
            case GOSSIP_ACTION_INFO_DEF+2:
                AddGossipItemFor(pPlayer, GOSSIP_ICON_CHAT, GOSSIP_ITEM_SKARLOC2, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+20);
                SendGossipMenuFor(pPlayer, GOSSIP_ID_SKARLOC2, pCreature->GetGUID());
                break;
            case GOSSIP_ACTION_INFO_DEF+20:
                SendGossipMenuFor(pPlayer, GOSSIP_ID_SKARLOC3, pCreature->GetGUID());
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        ...
        if(pInstance)
        {
            ...
            {
                AddGossipItemFor(pPlayer, GOSSIP_ICON_CHAT, GOSSIP_ITEM_WALKING, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
                SendGossipMenuFor(pPlayer, GOSSIP_ID_START, pCreature->GetGUID());
            }

            if(pInstance->GetData(TYPE_THRALL_PART1) == DONE && !pInstance->GetData(TYPE_THRALL_PART2))
            {
                AddGossipItemFor(pPlayer, GOSSIP_ICON_CHAT, GOSSIP_ITEM_SKARLOC1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
                SendGossipMenuFor(pPlayer, GOSSIP_ID_SKARLOC1, pCreature->GetGUID());
            }

            if(pInstance->GetData(TYPE_THRALL_PART2) == DONE && !pInstance->GetData(TYPE_THRALL_PART3))
            {
                AddGossipItemFor(pPlayer, GOSSIP_ICON_CHAT, GOSSIP_ITEM_TARREN, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+3);
                SendGossipMenuFor(pPlayer, GOSSIP_ID_TARREN, pCreature->GetGUID());
            }
        }
        return true;
    }
    ...
    {
        npc_thrall_old_hillsbradAI(Creature* pCreature) : EscortAI(pCreature)
        {
        ...
        void WaypointReached(uint32 i, uint32)
        {
            ...
            switch(i)
            {
                ...
                case 93:
                    ...
                    break;
                case 94:
                    if(pInstance->GetGuidData(DATA_TARETHA))
                    {
                        TarethaGUID.Set(pInstance->GetGuidData(DATA_TARETHA));
                ...
                case 106:
                    {
                        ...
                        if(Creature* Taretha = pInstance->instance->GetCreature(pInstance->GetGuidData(DATA_TARETHA)))
                        {
                            if(Player* player = GetPlayerForEscort())
                                CAST_AI(EscortAI, (Taretha->AI()))->Start(false, true, player->GetGUID());
                        }
...
public:
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        if(uiAction == GOSSIP_ACTION_INFO_DEF+1)
        {
            AddGossipItemFor(pPlayer, GOSSIP_ICON_CHAT, GOSSIP_ITEM_EPOCH2, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
            SendGossipMenuFor(pPlayer, GOSSIP_ID_EPOCH2, pCreature->GetGUID());
        }
        if(uiAction == GOSSIP_ACTION_INFO_DEF+2)
        {
            CloseGossipMenuFor(pPlayer);
            ...
            if(pInstance && pInstance->GetData(TYPE_THRALL_EVENT) == IN_PROGRESS)
            {
                ...
                if(pInstance->GetGuidData(DATA_EPOCH) == 0)
                     pCreature->SummonCreature(ENTRY_EPOCH, 2639.13f, 698.55f, 65.43f, 4.59f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 120000);
                    
                    if(pInstance->GetGuidData(DATA_THRALL))
                    {
                        ObjectGuid ThrallGUID;
                        ThrallGUID.Set(pInstance->GetGuidData(DATA_THRALL));
                        Creature* Thrall = (ObjectAccessor::GetCreature((*pCreature), ThrallGUID));
                        if(Thrall)
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        ...
        {
            AddGossipItemFor(pPlayer, GOSSIP_ICON_CHAT, GOSSIP_ITEM_EPOCH1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
            SendGossipMenuFor(pPlayer, GOSSIP_ID_EPOCH1, pCreature->GetGUID());
        }
        return true;
    }
    ...
    {
        npc_tarethaAI(Creature* pCreature) : EscortAI(pCreature)
        {
            pInstance = pCreature->GetInstanceScript();
        }
