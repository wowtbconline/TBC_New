    ...
    public:
        ...
        bool OnGossipHello(Player* pPlayer, GameObject* pGO)
        {
            ...
            if(pTyralius)
            {
                ...
                if(pPlayer)
                    pPlayer->KilledMonsterCredit(NPC_CAPTAIN_TYRALIUS);
...
public:
    ...
    struct npc_manaforge_control_consoleAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*pKiller*/)
        {
            ...
            if(someplayer)
            {
                Unit* p = ObjectAccessor::GetUnit((*me), someplayer);
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* pGo)
    {
        ...
        if(manaforge)
        {
            CAST_AI(npc_manaforge_control_console::npc_manaforge_control_consoleAI, manaforge->AI())->someplayer.Set(player->GetGUID());
            CAST_AI(npc_manaforge_control_console::npc_manaforge_control_consoleAI, manaforge->AI())->goConsole.Set(pGo->GetGUID());
    ...
    struct npc_commander_dawnforgeAI : public ScriptedAI
    {
        ...
        bool CanStartEvent(Player* player)
        {
            if(!isEvent)
            {
                ...
                ardonisGUID.Set(ardonis->GetGUID());
                PlayerGUID.Set(player->GetGUID());
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            case 6:
                ...
                {
                    case 0: //Subphase 1: Turn Dawnforge and Ardonis
                        Turn_to_Pathaleons_Image();
                        ++PhaseSubphase;
                        Phase_Timer = 8000;
                        break;
                    case 1: //Subphase 2 Dawnforge say
...
public:
    ...
    {
        mob_phase_hunterAI(Creature* pCreature) : ScriptedAI(pCreature) { }
...
public:
    ...
    bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest)
    {
        if(quest->GetQuestId() == Q_ALMABTRIEB)
        {
            ...
            CAST_AI(EscortAI, (creature->AI()))->Start(true, false, player->GetGUID());
        }
        return true;
    }
    ...
    {
        npc_bessyAI(Creature* pCreature) : EscortAI(pCreature) { }
...
class npc_windtradermarid : public CreatureScript
{
public:
    npc_windtradermarid() : CreatureScript("npc_windtradermarid") { }
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        if(uiAction == GOSSIP_ACTION_INFO_DEF+1)
        {
            if(Creature* pSummon = pCreature->SummonCreature(20071, 4280.453f, 2229.066f, 124.217f, 6.172f, TEMPSUMMON_TIMED_DESPAWN, 180000))
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        ...
        if(pPlayer->GetQuestStatus(10273) == QUEST_STATUS_INCOMPLETE)
            AddGossipItemFor(pPlayer, GOSSIP_ICON_CHAT, "Fight!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
...
public:
    ...
    struct npc_protectorate_demolitionistAI : public ScriptedAI
    {
        npc_protectorate_demolitionistAI(Creature* pCreature): ScriptedAI(pCreature) { }

        uint32 EventCheck_Timer;

        void Reset()
        {
            EventCheck_Timer = 2500;

            me->GetMotionMaster()->Clear();
            me->GetMotionMaster()->MoveIdle();
            if(Unit* pOwner = me->GetCharmerOrOwner())
                me->GetMotionMaster()->MoveFollow(pOwner, 1.0f, pOwner->GetOrientation());
        }

        void MoveInLineOfSight(Unit* pWho)
        {
            if(!me->GetCharmerOrOwner() && pWho && pWho->GetTypeId() == TYPEID_PLAYER && pWho->HasAura(35679)) {
                me->SetUInt64Value(UNIT_FIELD_SUMMONEDBY, pWho->GetGUID());
                // @todo me->SetCharmerGUID(pWho->GetGUID());    //WTF?? SetCharmedBy() ?
                me->GetMotionMaster()->MoveFollow(pWho, 1.0f, pWho->GetOrientation());
            }
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if(EventCheck_Timer <= diff)
            {
                ...
                {
                    // @todo me->SetCharmerGUID(0);  //hotfix. TODO: rewrite this shit completely with EscortAI!
                    me->DespawnOrUnsummon();
                }

                if(me->FindNearestCreature(20755, 20.0f))
                {
                    me->GetCharmerOrOwner()->ToPlayer()->CompleteQuest(10406);
                    me->RemoveCharmedBy(me->GetCharmerOrOwner());
                    me->DespawnOrUnsummon();
                }
...
void AddSC_netherstorm()
{
    ...
    new npc_windtradermarid;
    new npc_protectorate_demolitionist;
}
