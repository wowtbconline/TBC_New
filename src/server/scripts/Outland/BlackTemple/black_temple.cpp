...
enum Spells
{
	...
	// Illidari Nightlord
	SPELL_SHADOW_INFERNO_DAMAGES = 39646
};
...
class spell_illidari_nightlord_shadow_inferno : public AuraScript
{
	...
	{
		return ValidateSpellInfo({ SPELL_SHADOW_INFERNO_DAMAGES });
	}
	...
	void OnPeriodic(AuraEffect const* aurEffect)
	{
		...
		GetUnitOwner()->CastSpell(GetUnitOwner(), SPELL_SHADOW_INFERNO_DAMAGES, CastSpellExtraArgs(TRIGGERED_FULL_MASK).AddSpellBP0(bp));
	}
...
{
	return new spell_illidari_nightlord_shadow_inferno();
}
